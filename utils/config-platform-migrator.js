...
function sanitizeAppIdForPlatform(value, platform) {
  ...
  if (normalized === "xbox-pc" || normalized === "xbox-console") {
...
function inferOfficialPlatformFromMarkers(config = {}) {
  ...
  if (configPath.includes(`${schemaNeedle}xbox-console${path.sep}`)) {
    return "xbox-console";
  }
...
function migrateSchemaStorage({ configsDir, platformIndex, logger = console }) {
  ...
  for (const dir of legacyDirs) {
    ...
    const prefersXboxPc =
      ...
      !platforms?.has("gog") &&
      !platforms?.has("epic");
    const prefersXboxConsole =
      platforms?.has("xbox-console") &&
      !platforms?.has("steam") &&
      !platforms?.has("uplay") &&
      !platforms?.has("gog") &&
      !platforms?.has("epic");
